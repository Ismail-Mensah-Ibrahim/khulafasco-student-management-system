-- Migration: Fix student RPC parameter type mismatches
-- Corrects:
-- 1. update_student: Explicitly cast enum values to public.gender_type, public.student_type, public.enrollment_status in COALESCE
-- 2. delete_student: Pass v_student_id (uuid) directly to audit_logs.entity_id instead of casting to text

BEGIN;

-- 1. RPC: update_student
...
BEGIN
  ...
  SET
    ...
    gender = COALESCE(NULLIF(TRIM(p_gender), '')::public.gender_type, gender),
    ...
    house_id = p_house_id,
    student_type = COALESCE(NULLIF(TRIM(p_student_type), '')::public.student_type, student_type),
    academic_year_id = COALESCE(p_academic_year_id, academic_year_id),
    enrollment_status = COALESCE(NULLIF(TRIM(p_enrollment_status), '')::public.enrollment_status, enrollment_status),
...
BEGIN
  ...
  -- 4. Log deletion in audit_logs (audit_logs.entity_id is uuid)
